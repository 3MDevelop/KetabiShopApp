...
export default function Login() {
  ...
  const getAuthCode = async () => {
    if (!mobile || mobile.length < 11) {
      showToast("error", "خطا", "لطفاً شماره موبایل معتبر وارد کنید");
      return;
    }
    if (!isAccepted) {
    ...
    try {
      ...
      if (response.ok) {
        showToast("success", "کد تایید", "کد تایید برای شما ارسال شد");
        setShowCodeInput(true);
      } else {
        showToast("error", "خطا", data.message || "مشکلی در ارسال کد پیش آمد");
      }
    } catch (error) {
      console.info(error)
  ...
  const verifyCode = async () => {
    if (!authCode || authCode.length < 4) {
      showToast("error", "خطا", "لطفاً کد تایید را وارد کنید");
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        await login(mobile, authCode);
        router.replace("/");
      } else {
        showToast("error", "خطا", data.message || "کد تایید صحیح نیست");
      }
    } catch (error) {
      console.info(error)
      showToast("error", "خطا", "مشکل در ارتباط با سرور");
