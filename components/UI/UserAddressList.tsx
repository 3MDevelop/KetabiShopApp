  ...
  return (
    <View style={styles.infoChip}>
      <CustomText
        variant="caption"
        bold
        style={[styles.infoChipText, large && styles.infoChipTextLarge]}
      >
        {label}
      </CustomText>
    </View>
...
export default function UserAddressList() {
  ...
  const loadAddresses = useCallback(async () => {
    ...
    try {
      setAddresses(await fetchAddresses(user.phone, user.token));
    } catch {
      Toast.show({
        type: "error",
  ...
  const selectProvince = async (item: PlaceItem) => {
    setForm((prev) => ({ ...prev, provinceId: item.id, cityId: "" }));
    setCities([]);
    setOpenSelect(null);
    try {
  ...
  const saveAddress = async () => {
    ...
    try {
      const result = await createAddress({
        ownerMobile: user.phone,
        mobile: form.recipientMobile.trim(),
          ...
          return (
            <View key={item.id} style={styles.addressCard}>
              <View style={styles.chipRow}>
                {title ? <InfoChip label={title} large /> : null}
                {place ? <InfoChip label={place} /> : null}
              </View>
              {previewAddress ? (
              ...
              <View style={styles.chipRow}>
                {recipientMobile ? <InfoChip label={recipientMobile} /> : null}
                {recipient ? <InfoChip label={recipient} /> : null}
              </View>
            </View>
          );
        })
        ...
        <View style={styles.modalBackdrop}>
          <View style={styles.modalCard}>
            ...
            >
              ...
              <View style={[styles.formRow, isRTL && styles.formRowReverse]}>
                ...
                <View style={styles.formRowInput}>
                  <SelectField
                    ...
                    onSelect={(item) => {
                      setForm((prev) => ({ ...prev, cityId: item.id }));
                      setOpenSelect(null);
                    }}
                    textAlign={textAlign}
                    fontFamily={fontFamily}
              ...
              <View style={[styles.formRow, isRTL && styles.formRowReverse]}>
                <TextInput
                  style={[styles.input, styles.formRowInput, { textAlign, fontFamily }]}
